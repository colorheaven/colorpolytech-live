...
function poptions($table,$nameCandidates){
    if(!table_exists($table))return[];$name=ppick($nameCandidates,$table);if(!$name)return[];
    try{return db()->query("SELECT id, `$name` name FROM `$table` ORDER BY `$name`")->fetchAll();}catch(Throwable $e){return[];}
}
...
try{
    if($_SERVER['REQUEST_METHOD']==='POST'){
        check_csrf();$id=(int)($_POST['id']??0);require_perm('products.'.($id?'edit':'add'));
...
<?php if($error):?><div class="alert alert-danger"><?=e($error)?></div><?php endif;?><?php if(!empty($_GET['msg'])):?><div class="alert alert-success">Product <?=e($_GET['msg'])?> successfully.</div><?php endif;?>
...
<div class="card"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead><tr><th>Product</th><th>Code/SKU</th><th>Base Unit</th><th>Alt Unit</th><th class="text-end">Current Stock</th><th class="text-end">Sales Rate</th><th>Status</th><th class="text-end">Action</th></tr></thead><tbody><?php foreach($rows as$r):?><tr><td><strong><?=e(pval($r,$nameCol))?></strong><br><small class="text-muted"><?=e(pval($r,$supplierGradeCol))?></small></td><td><?=e(pval($r,$codeCol))?><br><small><?=e(pval($r,$skuCol))?></small></td><td><?=e(pval($r,$unitCol))?></td><td><?=e(pval($r,$altUnitCol))?> <small><?=e(pval($r,$baseQtyCol))?> base/unit</small></td><td class="text-end"><?=money(pval($r,$currentStockCol,0))?></td><td class="text-end"><?=money(pval($r,$salesRateCol,0))?></td><td><?=e(pval($r,$statusCol))?></td><td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="products.php?action=form&id=<?=$r['id']?>">Edit</a></td></tr><?php endforeach;?><?php if(!$rows):?><tr><td colspan="8" class="text-center text-muted py-4">No products found.</td></tr><?php endif;?></tbody></table></div></div>
