document.addEventListener('DOMContentLoaded', function () {
  ...
  function isFormField(el) { return el && el.matches && el.matches('input, select, textarea'); }
  function textOf(el) { return (el && el.textContent ? el.textContent : '').trim(); }
  function isSuggestionText(txt) { return /stock\s*:|due\s*:|masterbatch|pigment|kilogram|017|\btk\b|\d{4}/i.test(txt || ''); }
  function hideBox(el) {
  ...
  function findSuggestionBox(item) {
    var el = item;
    for (var i = 0; i < 7 && el && el !== document.body; i++, el = el.parentElement) {
      var idClass = ((el.id || '') + ' ' + (el.className || '')).toString();
      var txt = textOf(el);
      var fields = el.querySelectorAll ? el.querySelectorAll('input,select,textarea').length : 0;
      ...
      if (children >= 2 && fields <= 1 && txt.length > 25 && isSuggestionText(txt)) return el;
    }
    return item.parentElement;
  }
  function hideAllSuggestionBoxes() {
    document.querySelectorAll('[class*="suggest" i], [id*="suggest" i], [class*="autocomplete" i], [id*="autocomplete" i], [class*="lookup" i], [id*="lookup" i], [class*="result" i], [id*="result" i], [class*="typeahead" i], [id*="typeahead" i], .dropdown-menu').forEach(function (box) {
      var txt = textOf(box);
      if (isSuggestionText(txt) || /suggest|autocomplete|lookup|result|typeahead/i.test(((box.id || '') + ' ' + (box.className || '')).toString())) hideBox(box);
    });
  }
  document.addEventListener('click', function (event) {
    if (isFormField(event.target)) return;
    var item = event.target.closest ? event.target.closest('button, a, div, span, li, strong, small') : event.target;
    var txt = textOf(item);
    if (!txt || !isSuggestionText(txt)) return;
    setTimeout(function () {
      hideBox(findSuggestionBox(item));
      hideAllSuggestionBoxes();
    }, 180);
  }, false);
  document.addEventListener('keydown', function (event) {
    if (event.key === 'Escape') hideAllSuggestionBoxes();
  });
});
